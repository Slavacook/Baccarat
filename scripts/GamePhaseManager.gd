...
func on_action_pressed():
	...
	if state == GameStateManager.GameState.CHOOSE_WINNER:
		...
		if not payout_queue_manager or not payout_queue_manager.has_any_payouts():
			...
			# После валидации победителя формируется очередь выплат
			# Проверяем еще раз: если выплат так и нет (все переключатели выключены) → завершаем раунд
			if payout_queue_manager and not payout_queue_manager.has_any_payouts():
				print("==================================================")
				print("✅ ВСЕ ПЕРЕКЛЮЧАТЕЛИ ВЫКЛЮЧЕНЫ → ЗАВЕРШАЕМ РАУНД БЕЗ ВЫПЛАТ")
				print("==================================================")
				EventBus.show_toast_info.emit("Нет активных ставок. Начинаем новый раунд.")
				# Продолжаем к завершению раунда (не return)
			else:
				# Есть выплаты для оплаты → ждем следующего нажатия кнопки
				return

		# ← НОВОЕ: Если в очереди есть ставки, но НЕТ выигравших (все проиграли)
		# → сразу завершаем раунд без выплат
		elif payout_queue_manager and payout_queue_manager.has_any_payouts() and not payout_queue_manager.has_unpaid_winnings():
			print("==================================================")
			print("✅ НЕТ ВЫИГРЫШНЫХ СТАВОК → ЗАВЕРШАЕМ РАУНД БЕЗ ВЫПЛАТ")
			print("==================================================")
			EventBus.show_toast_info.emit("Нет выигрышных ставок. Начинаем новый раунд.")
			# Переходим к завершению раунда (тот же код что и для "все оплачено")
			# Продолжаем выполнение ниже (не return)
		else:
			# Есть неоплаченные выплаты → этот случай уже обработан выше
