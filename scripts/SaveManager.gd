...
# ← Настройки уровня громкости фоновой музыки (0-5)
func save_background_music_level(level: int):
	"""Сохранить уровень громкости фоновой музыки (0 = выкл, 1-5 = громкость 0.1-0.5)"""
	var settings = load_settings()
	settings["background_music_level"] = clamp(level, 0, 5)
	save_settings(settings)
	
	# Удаляем старое значение bool для обратной совместимости
	if settings.has("background_music_enabled"):
		settings.erase("background_music_enabled")
		save_settings(settings)

func load_background_music_level() -> int:
	"""Загрузить уровень громкости фоновой музыки (0-5)
	
	Обратная совместимость: если есть старое значение bool, конвертируем:
	- true → уровень 3
	- false → уровень 0
	
	По умолчанию: уровень 3 (громкость 0.3)
	"""
	var settings = load_settings()
	
	# Проверяем, есть ли новое значение уровня
	if settings.has("background_music_level"):
		return settings.get("background_music_level", 3)
	
	# Обратная совместимость: конвертируем старое bool значение
	if settings.has("background_music_enabled"):
		var old_enabled = settings.get("background_music_enabled", true)
		var converted_level: int
		if old_enabled:
			converted_level = 3
		else:
			converted_level = 0
		
		# Сохраняем конвертированное значение и удаляем старое
		settings["background_music_level"] = converted_level
		settings.erase("background_music_enabled")
		save_settings(settings)
