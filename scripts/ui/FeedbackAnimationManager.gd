...
const END_Y_OFFSET: float = -80.0  # Смещение конечной позиции Y относительно начальной (отрицательное = выше)
...
const MOVE_DURATION: float = 5.0  # Длительность движения вверх (секунды)
const FADE_DURATION: float = 4.5  # Длительность fade out (секунды) - должна быть меньше MOVE_DURATION
const TOTAL_DURATION: float = 5.0  # Общая длительность показа оповещения (секунды)
