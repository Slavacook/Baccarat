...
func _ready():
	...
	# Отключаем фокус - кнопка управляется только мышью и клавишей F
	# Это предотвращает активацию кнопки пробелом после клика мышью
...
func toggle_state():
	"""Переключить состояние (публичный метод для вызова извне, например, с геймпада)"""
	# Просто вызываем _on_pressed(), который уже обрабатывает переключение и эмитит сигнал
	_on_pressed()
