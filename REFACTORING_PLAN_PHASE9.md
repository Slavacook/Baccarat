# План рефакторинга ФАЗА 9: Дальнейшее уменьшение GameController

**Цель**: Уменьшить `GameController` с ~2006 строк до < 1500 строк  
...
### 1. PayoutReturnHandler (~200 строк) 🔴 Высокий приоритет
...
- `survival_state: SurvivalStateProvider`
...
### 2. PayoutPreparationHandler (~150 строк) 🔴 Высокий приоритет
...
### 3. UIEventHandler (~150 строк) 🟡 Средний приоритет
...
- `_hide_game_ui_elements()` - скрытие UI элементов игры
- `_show_game_ui_elements()` - показ UI элементов игры
...
- **GameController**: ~1350 строк (было 2006) ✅ **-33%**
- **PayoutReturnHandler**: ~200 строк (новый)
- **PayoutPreparationHandler**: ~150 строк (новый)
- **UIEventHandler**: ~150 строк (новый)
- **InputHandler**: ~150 строк (новый)

**Всего извлечено**: ~650 строк

---

## 🎯 План выполнения

### Шаг 1: Извлечь PayoutReturnHandler
...
### Шаг 2: Извлечь PayoutPreparationHandler
